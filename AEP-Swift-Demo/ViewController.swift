...
//        AEPTarget.getTntId({ (id) in
//            print("AEPTarget.getTntId \(String(describing: id))")
//        })
...
extension ViewController{
    ...
    @objc func applyTargetOffers(){
        ...
        // TEST: loop through 3 a4t locations
        for testLocation in ["pref-a4t-location-1","pref-a4t-location-2","pref-a4t-location-3"] {
            print("\(testLocation)")
            AEPSDKManager.getPrefetchedLocation(forKey: .HomePage, location: testLocation) { (content) in
                print("prefetched content \(String(describing: testLocation)) \(String(describing: content))")
            }
        }
